...
export default async function(appHandle) {
  ...
  const prom = publicIds.map( async (publicId) => {
    ...
    const keysHandle = await window.safeMutableData.getKeys(servicesHandle);

    await window.safeMutableDataEntries.forEach(keysHandle, (key) => {
      rawWalletIds.push(key.toString());
    });
